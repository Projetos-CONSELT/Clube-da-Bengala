...
export default function Equipamentos() {
  ...
  const handleSave = () => {
    if (!formData.codigo_patrimonio || !formData.tipo_id) return;
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-1 text-sm text-slate-500">
                            {getAtributosEquipamento(eq.atributos_especificos).localizacao && (
                              <span className="flex items-center gap-1">
                                <MapPin className="w-3 h-3" /> {String(getAtributosEquipamento(eq.atributos_especificos).localizacao)}
                              </span>
                            )}
                            <Badge className="bg-blue-100 text-blue-700">
                              {String(getAtributosEquipamento(eq.atributos_especificos).estado_conservacao || 'bom')}
                            </Badge>
                          </div>
                        </div>
                      </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              disabled={isSaving || !formData.codigo_patrimonio || !formData.tipo_id}
            >
